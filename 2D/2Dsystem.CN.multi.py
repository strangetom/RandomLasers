...
for timestep in range(N):
	...
	result1 = pool.apply_async(np.linalg.solve, [A_pump, PUMP_RHS(W_G, N_mid_step, I_G_mid_step)] )
	result2 = pool.apply_async(np.linalg.solve, [A_probe, PROBE_RHS(W_R, N_mid_step, I_R_mid_step)] )
	result3 = pool.apply_async(np.linalg.solve, [A_ASE, ASE_RHS(W_A, N_mid_step)] )

	W_G_mid_step = result1.get()
	W_R_mid_step = result2.get()
	W_A_mid_step = result3.get()
